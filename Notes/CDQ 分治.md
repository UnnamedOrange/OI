...
​	以上方法不受空间范围影响，不受偏序的限制。我们可以这样认为：**归并排序利用分治的特性（将问题简单化后再合并）降低了维度。**类似于这样的解决办法，我们称为 CDQ 分治。
...
##### e.g. [51NOD 1376 最长递增子序列的数量](http://www.51nod.com/onlineJudge/questionCode.html#!problemId=1376)
...
​	我们可以在 $O(n \log n)$ 的时间复杂度内求出 $f$。考虑求 $g$，它的转移实际上是同时满足两个条件的 $g_j$ 之和：条件一是 $j < i$，条件二是 $f_j + 1 = f_i$。
...
​	上面这份代码跟之前的代码显然不同，而且根本不以归并排序为框架，只是用了一下归并排序的板子。

​	事实上，虽然前面说 CDQ 分治基于归并排序，但是它的核心思想是**只考虑左边部分对右边部分的贡献**，因此它的特点是：
...
​	注意代码中的细节，为什么只能用小于号，不能用小于等于号呢？**因为用小于等于号会导致算错：因为可能有重复的数，所以用小于等于号可能把不下降序列也当作了上升序列。**所以这里必须用小于号。
