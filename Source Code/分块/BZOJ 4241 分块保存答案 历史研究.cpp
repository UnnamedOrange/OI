#include <cstdio>
#include <cstdlib>
#include <cmath>
#include <cstring>
#include <iostream>
#include <algorithm>
#include <vector>
#include <string>
#include <stack>
#include <queue>
#include <deque>
#include <map>
#include <set>
#include <bitset>
#include <list>
typedef int INT;
using std::cin;
using std::cout;
using std::endl;
INT readIn()
{
	INT a = 0;
	bool minus = false;
	char ch = getchar();
	while (!(ch == '-' || (ch >= '0' && ch <= '9'))) ch = getchar();
	if (ch == '-')
	{
		minus = true;
		ch = getchar();
	}
	while (ch >= '0' && ch <= '9')
	{
		a = a * 10 + (ch - '0');
		ch = getchar();
	}
	if (minus) a = -a;
	return a;
}
void printOut(long long x)
{
	char buffer[20];
	INT length = 0;
	if (x < 0)
	{
		putchar('-');
		x = -x;
	}
	do
	{
		buffer[length++] = x % 10 + '0';
		x /= 10;
	} while (x);
	do
	{
		putchar(buffer[--length]);
	} while (length);
	putchar('\n');
}

const INT maxn = INT(1e5) + 5;
const INT maxN = 330;
INT n, q;
INT a[maxn];
INT bound;
INT disc[maxn];

INT N, sqrtN;
inline INT code(INT x)
{
	return (x - 1) / sqrtN;
}
INT inBlock[maxn];
INT lStart[maxN];
INT rEnd[maxN];
INT times[maxN][maxn];
long long ans[maxN][maxN];

void discretize()
{
	for (int i = 1; i <= n; i++)
		disc[i] = a[i];
	std::sort(disc + 1, disc + 1 + n);
	bound = std::unique(disc + 1, disc + 1 + n) - (disc + 1);
	for (int i = 1; i <= n; i++)
		a[i] = std::lower_bound(disc + 1, disc + 1 + bound, a[i]) - disc;
}

INT appear[maxn];
INT buf[maxn];

void run()
{
	n = readIn();
	q = readIn();
	for (int i = 1; i <= n; i++)
		a[i] = readIn();
	discretize();
	sqrtN = std::sqrt(n);
	N = (n + sqrtN - 1) / sqrtN;

	//预处理分块信息
	for (int i = 1; i <= n; i++)
	{
		INT ib = inBlock[i] = code(i);
		if (!lStart[ib])
			lStart[ib] = i;
		rEnd[ib] = i;
	}

	//预处理暴力计算时从整块中提取的信息
	for (int i = 1; i <= n; i++)
		times[inBlock[i]][a[i]]++;
	for (int i = 1; i < N; i++)
		for (int j = 1; j <= bound; j++)
			times[i][j] += times[i - 1][j];

	//预处理整块答案
	for (int i = 0; i < N; i++)
	{
		buf[0] = 0;
		long long t = 0;
		for (int j = lStart[i]; j <= n; j++)
		{
			appear[a[j]]++;
			t = std::max(t, (long long)appear[a[j]] * disc[a[j]]);
			if (j == rEnd[inBlock[j]])
				ans[i][inBlock[j]] = t;
		}
		memset(appear, 0, sizeof(appear));
	}

	while (q--)
	{
		INT l = readIn();
		INT r = readIn();
		long long t = 0;
		buf[0] = 0;
		if (inBlock[l] == inBlock[r])
		{
			for (int i = l; i <= r; i++)
			{
				if (!appear[a[i]])
					buf[++buf[0]] = a[i];
				appear[a[i]]++;
				t = std::max(t, (long long)appear[a[i]] * disc[a[i]]);
			}
		}
		else
		{
			t = ans[inBlock[l] + 1][inBlock[r] - 1];
			for (int i = l, to = rEnd[inBlock[l]]; i <= to; i++)
			{
				if (!appear[a[i]])
					buf[++buf[0]] = a[i];
				appear[a[i]]++;
			}
			for (int i = lStart[inBlock[r]]; i <= r; i++)
			{
				if (!appear[a[i]])
					buf[++buf[0]] = a[i];
				appear[a[i]]++;
			}
			for (int i = 1; i <= buf[0]; i++)
			{
				appear[buf[i]] += times[inBlock[r] - 1][buf[i]] - times[inBlock[l]][buf[i]];
				t = std::max(t, (long long)appear[buf[i]] * disc[buf[i]]);
			}
		}
		while (buf[0])
			appear[buf[buf[0]--]] = 0;
		printOut(t);
	}
}

int main()
{
	run();
	return 0;
}