...
&emsp;&emsp;由于不能旋转，所以*无旋 Treap*（又称 *fhq Treap*，以纪念引入人）必须有自己独特的操作。它们分别是 split 操作 和 merge 操作。
...
[^图片来源]: 这是一张 gif 动图。图片来源：[fhq treap 总结](http://www.yhzq-blog.cc/fhq-treap%E6%80%BB%E7%BB%93/#1_split)
...
&emsp;&emsp;我们定义 $\mathrm{split}(x, k)$ 为将 Treap 结点 $x$ 分裂成两个 Treap，一个包含 $x$ 的中序遍历的前 $k$ 个元素，另一个包含 $x$ 的后 $size_x - k$ 个元素。
...
&emsp;&emsp;根据需要，我们可能会有两种 split 操作：将**小于等于** $x$ 的结点分成左 Treap（按权值）；将前 $k$ 个结点分成左 Treap（按子树大小）。因为在仅把 Treap 当作集合时一般只需要看权值，所以下面我们以按权值为例讲解。

------

&emsp;&emsp;我们设 $(a, b) = \mathrm{split}(root, x)$。

&emsp;&emsp;如果我们有 $x < val_{root}$，那么我们可以知道 $a$ 是左子树的一部分，$b$ 至少包含了 $root$ 和右子树。令 $(c, d) = \mathrm{split}(lc_{root}, x)$，那么 $a = c$；$b$ 的根和右子树就是原来的 $root$ 及其右子树，左子树为 $d$。 

&emsp;&emsp;当 $x > val_{root}$ 时同理（我们把 $x = val_{root}$ 的情况归类到 $x > val_{root}$ 中，即相等的值全部都要分到左边去），左边至少由 $root$ 及其左子树组成，继续递归处理。

&emsp;&emsp;如果需要按排名分裂，操作是类似的，只不过再递归右子树时需要减去相应的排名。
...
&emsp;&emsp;由于本文的重点是可持久化数据结构，因此不再实现更多的无旋 Treap 的功能，在下面可持久化部分我们再来研究其它实现。另外，上面的代码只是提供了最基本的思路，**没有进行任何容错**，请务必注意。

###### ④无旋 Treap 的时间复杂度

&emsp;&emsp;由于树高是期望 $O(\log n)$ 的（满足 BST 和堆的性质，形态唯一），所以时间复杂度为期望 $O(n \log n)$。

&emsp;&emsp;实际上无旋 Treap 的常数较大，运行时间离 Treap 还差一个档次，但由于它无需旋转，允许可持久化，所以有着不可代替的作用。
