...
&emsp;&emsp;广义欧拉筛[^广义欧拉筛]指，使用欧拉筛法求出各种**积性**函数，而不仅仅局限于筛出素数或者欧拉函数。
...
&emsp;&emsp;对于广义欧拉筛，只需要能够快速计算 $f(p^k)$，就基本可以使用了，因为我们可以通过
$$
f(np) = f(\frac {n} {p^{k - 1}}) f(p^k) \qquad (p \mid n)
$$
来快速计算 $f(np)$。

&emsp;&emsp;具体实现时，这样做会造成一些麻烦：计算 $f(p^k)$ 的时间复杂度往往不会是 $O(1)$，且记录每个数的最小因子的出现次数也略显麻烦。实际上，我们需要的是一个递推公式：
$$
f(p^{k + 1}) = f(p^k) \cdot F(p)
$$
&emsp;&emsp;$F(p)$ 往往能够在 $O(1)$ 的时间复杂度内计算，所以我们有：
$$
f(s) =
\left\{
\begin{matrix}
...
\qquad(s = np)
$$
&emsp;&emsp;可以知道质数个数是 $O(\frac {n} {\log n})$ 级别的。只要计算 $f(p)$ 的时间复杂度小于等于 $O(\log n)$，计算 $F(p)$ 的时间复杂度为 $O(1)$，就能够在线性时间复杂度内计算出积性函数。
