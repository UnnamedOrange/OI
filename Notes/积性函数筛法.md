...
也是拿来求积性函数前缀和的，但适用范围比杜教筛广多了（参见[适用范围](#4.-适用范围)）。除此之外，它还能筛一些别的函数的前缀和。
...
即：无论 $i$ 是不是质数，在 $f'(i)$ 中我们都把 $i$ 当作一个质数代入 $f(i)$ 中计算。例如：
...
首先 $s(n, k + 1) = g(n, k)$。注意到，我们把 $g$ 拆成两部分的理由是要保证我们求的东西是*完全*积性函数，而这里我们只用保证 $s$ 是积性函数就可以了，因此我们把所有的 $g$ 合并起来即可。

如果你直接像我第③点这么做，你就会 WA 得很惨。注意，既然我们把 $g$ 拆成两部分的理由是保证 $g_{1 \sim 2}$ 都是完全积性函数，那么你验证一下，$g_1(x) = -1$ 是完全积性函数吗？
$$
g_1(a) g_1(b) = (-1) \cdot (-1) = 1 \ne g_1(ab)
$$
根据 $g_1(x)$ 的定义，$g_1(ab) = -1$。

所以拆开后要检查一下 $g_i$ 是否为积性函数。一般来说，$g$ 是一个多项式。拆开时，如果前面是负号，就一定不能带有负号，只能把正号带进去，最后合并时再取相反数。

###### ⑤求 $s$ 的时间复杂度

可以认为时间复杂度仍然是 $O(\frac {n^{\frac {3} {4}}} {\log n})$。


##### 4. 适用范围

对于积性函数 $f$，如果对于一个质数 $p$，$f(p)$ 的表达式是一个完全积性函数或者多个完全积性函数的和（比如一个多项式），并且 $f(x^k)$ 能够在 $O(1)$ 的时间复杂度内求出（或者在 $O(k)$ 的时间复杂度内**递推**求出），那么就可以用 Min_25 筛求 $f$ 的前缀和。（如果你是跳跃着阅读到这的，你可以认为：Min_25 筛能够求绝大部分积性函数的前缀和。）
...
下面的练习可能可以使用别的筛法通过，但是这里都用 Min_25 筛实现。有些练习可能会介绍一些新知识点。
