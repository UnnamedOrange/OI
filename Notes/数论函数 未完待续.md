...
&emsp;&emsp;现在我们可以用一个正确的方法在规定时间内处理一个询问了。然而因为有多组询问，这仍然不够快，特别是枚举质数这一操作还有待改进。我们将在[例题（Part 2）](#例题（part-2）)中继续讨论。

------

&emsp;&emsp;上例中，较难处理的是 $[\gcd(x, y) = 1]$，我们实际上是利用了 $\epsilon = \mu * 1$，使用 $\sum_{d \mid \gcd(x, y)} \mu(d)$ 表示了 $\gcd(x, y)$，这也是一种莫比乌斯反演。

>   &emsp;&emsp;还可以使用 $\varphi * 1 = \mathrm{id}$ 进行运算的简化，出于方便，我们也称它为莫比乌斯反演。（我确实不是很明白莫比乌斯反演的具体定义）
